...
it('calls correct endpoint', function () {
    $request = new CreateStockMarketOrderRequest(
        'WBT_USDT',
        OrderTypeEnum::BUY,
        '6',
    );
    ...
    expect($request->resolveEndpoint())
        ->toBe('/api/v4/order/stock_market')
...
test('status code is correct', function () {
    ...
    $connector->withMockClient(new MockClient([
        new BasicFixture('/api/v4/order/stock_market'),
    ]));

    $result = $connector->send(new CreateStockMarketOrderRequest(
        'WBT_USDT',
        OrderTypeEnum::BUY,
        '6',
    ));
